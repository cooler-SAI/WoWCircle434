...
DECLARE cur CURSOR FOR SELECT `character_currency`.`guid`, `character_currency`.`currency`, `character_currency`.`total_count` FROM `character_currency` WHERE `character_currency`.`currency` IN (390, 392) ORDER BY `character_currency`.`guid` ASC, `character_currency`.`currency` DESC;
