...
UPDATE  `creature_template` SET  `ScriptName` =  'npc_nightmare_illusion' WHERE  `entry` = 15163;
...
UPDATE  `creature_template` SET  `ScriptName` =  'mob_twilight_zealot' WHERE `entry` = 50284;
UPDATE  `creature_template` SET  `ScriptName` =  'npc_tol_barad_battlemage' WHERE `subname` = 'Tol Barad Battle-Mage';
UPDATE  `creature_template` SET  `ScriptName` =  'npc_tb_spiritguide' WHERE `entry` IN (45079, 45066);
...
(96646, 'spell_renataki_thousand_blades'),
