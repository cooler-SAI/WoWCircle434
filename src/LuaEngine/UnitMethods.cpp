...
int LuaUnit::IsHeroic(lua_State* L, Unit* unit)
{
    TO_UNIT_BOOL();

    sEluna->PushBoolean(L, unit->GetMap()->IsHeroic());
    return 1;
}
...
int LuaUnit::ModifyCurrency(lua_State* L, Unit* unit)
{
    TO_PLAYER();

    int32 currency = luaL_checkunsigned(L, 1);
    int32 amount = luaL_checkunsigned(L, 2);

    player->ModifyCurrency(currency, amount);
    return 0;
}
...
int LuaUnit::ResetTalentsCost(lua_State* L, Unit* unit)
{
    ...
    sEluna->PushUnsigned(L, player->GetTalentResetCost());
    return 1;
}
...
int LuaUnit::CastedCreatureOrGO(lua_State* L, Unit* unit)
{
    ...
    uint64 spellId = sEluna->CHECK_ULONG(L, 3);
    player->CastedCreatureOrGO(entry, guid, spellId);
    return 0;
}
...
int LuaUnit::RemovePet(lua_State* L, Unit* unit)
{
    ...
    int mode = luaL_optint(L, 1, PET_SLOT_DELETED);
    bool returnreagent = luaL_optbool(L, 1, false);

    if (!player->GetPet())
        return 0;

    player->RemovePet(player->GetPet(), (PetSlot)mode, returnreagent);
    return 0;
}
...
int LuaUnit::GetCurrency(lua_State* L, Unit* unit)
{
    TO_PLAYER();

    uint32 currency = luaL_checkunsigned(L, 1);

    sEluna->PushUnsigned(L, player->GetCurrency(currency, true));
    return 1;
}
...
int LuaUnit::MoveTo(lua_State* L, Unit* unit)
{
    ...
    unit->GetMotionMaster()->MovePoint(id, x, y, z);
    return 0;
}
...
int LuaUnit::SetCurrency(lua_State* L, Unit* unit)
{
    TO_PLAYER();
...
int LuaUnit::HandleGameObject(lua_State* L, Unit* unit)
{
    TO_UNIT();

    bool open = luaL_optbool(L, 1, true);
    GameObject* go = sEluna->CHECK_GAMEOBJECT(L, 2);
    if (!go)
        return 0;
