...
void GameEventMgr::StopEvent(uint16 event_id, bool overwrite)
{
    ...
    else if (serverwide_evt)
    {
        ...
        if (overwrite || data.state != GAMEEVENT_WORLD_FINISHED)
        {
            ...
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_DEL_ALL_GAME_EVENT_CONDITION_SAVE);
            stmt->setUInt8(0, uint8(event_id));
            trans->Append(stmt);

            stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_DEL_GAME_EVENT_SAVE);
            stmt->setUInt8(0, uint8(event_id));
            trans->Append(stmt);
    ...
    if (itr != mQuestToEventConditions.end())
    {
        ...
        if (citr != mGameEvent[event_id].conditions.end())
        {
            ...
            if (citr->second.done < citr->second.reqNum)
            {
                ...
                PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<2>(CHAR_DEL_GAME_EVENT_CONDITION_SAVE);
                stmt->setUInt8(0, uint8(event_id));
                stmt->setUInt32(1, condition);
                trans->Append(stmt);

                stmt = CharacterDatabase.GetPreparedStatement<3>(CHAR_INS_GAME_EVENT_CONDITION_SAVE);
                stmt->setUInt8(0, uint8(event_id));
                stmt->setUInt32(1, condition);
...
void GameEventMgr::SaveWorldEventStateToDB(uint16 event_id)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_DEL_GAME_EVENT_SAVE);
    stmt->setUInt8(0, uint8(event_id));
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement<3>(CHAR_INS_GAME_EVENT_SAVE);
    stmt->setUInt8(0, uint8(event_id));
