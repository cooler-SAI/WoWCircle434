        ...
        for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
            if (Player* member = itr->getSource())
                if (member->isAlive() && member->IsAtGroupRewardDistance(_victim))
                {
                    ...
                    // 2.4. _maxNotGrayMember - maximum level of alive group member within reward distance,
                    //      for whom victim is not gray;
                    uint32 grayLevel = CerberCore::XP::GetGrayLevel(lvl);
...
inline void KillRewarder::_InitXP(Player* player)
{
    ...
    if (_isBattleGround || (!_isPvP && !_killer->GetVehicle()))
        _xp = CerberCore::XP::Gain(player, _victim);
}
    ...
    if (_maxLevel)
    {
        ...
        if (!_isBattleGround || _xp)
        {
            ...
            if (!_isBattleGround)
            {
                ...
                _groupRate = CerberCore::XP::xp_in_group_rate(_count, isRaid);
            }
...
void Player::Update(uint32 p_time)
{
    ...
    // check every second
    if (now > m_Last_tick + 1*IN_MILLISECONDS)
...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (isInCombat()) // CerberCore Updates Mana in intervals of 2s, which is correct
...
void Player::RegenerateHealth()
{
    ...
    else if (!incombat || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT))
    {
...
void Player::XPGainAborted()
{
    ...
    WorldPacket data(SMSG_XP_GAIN_ABORTED, 8 + 4 + 4 + 4);
    ...
    GetSession()->SendPacket(&data);
    // xp/xpgain - one of unk's. Rest? Order?
}
...
bool Player::AddTalent(uint32 spellId, uint8 spec, bool learning)
{
    ...
    if (!spellInfo)
    {
        ...
        if (!IsInWorld() && !learning)                       // spell load case
        {
            ...
            CharacterDatabase.PExecute("DELETE FROM character_talent WHERE spell = '%u'", spellId);
        }
        else
            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetCerberCoreStringForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
bool Player::UpdateCraftSkill(uint32 spellid)
{
    ...
    for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first; _spell_idx != bounds.second; ++_spell_idx)
    {
        if (_spell_idx->second->skillId)
        {
            ...
            int32 rate = sWorld->getIntConfig(CONFIG_SKILL_GAIN_CRAFTING);
            uint32 craft_skill_gain = rate;
            uint32 points = _spell_idx->second->rewardSkillPoints;
            if (rate == 1 && points > 0)
...
void Player::SendMessageToSetInRange(WorldPacket* data, float dist, bool self)
{
    ...
    CerberCore::MessageDistDeliverer notifier(this, data, dist);
...
void Player::SendMessageToSetInRange(WorldPacket* data, float dist, bool self, bool own_team_only)
{
    ...
    CerberCore::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    VisitNearbyWorldObject(dist, notifier);
}
...
void Player::SendMessageToSet(WorldPacket* data, Player const* skipped_rcvr)
{
    ...
    CerberCore::MessageDistDeliverer notifier(this, data, GetVisibilityRange(), false, skipped_rcvr);
...
int32 Player::CalculateReputationGain(uint32 creatureOrQuestLevel, int32 rep, int32 faction, bool for_quest, bool noQuestBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= CerberCore::XP::GetGrayLevel(getLevel()))
        percent *= rate;
...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        {
            if (InBattleground() && GetBGTeam() == plrVictim->GetBGTeam())
                return false;

            if (!InBattleground() && GetTeam() == plrVictim->GetTeam() && !sWorld->IsFFAPvPRealm())
                return false;

            uint8 k_level = getLevel();
            uint8 k_grey = CerberCore::XP::GetGrayLevel(k_level);
            ...
            honor_f = ceil(CerberCore::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
    ...
    // add honor points
    ModifyCurrency(CURRENCY_TYPE_HONOR_POINTS, int32(honor * 100.0f));
...
void Player::_LoadCurrency(PreparedQueryResult result)
{
    ...
    do
    {
        ...
        if (currency->Category == CURRENCY_CATEGORY_META_CONQUEST)
        {
            ...
            if (cap > _ConquestCurrencytotalWeekCap / 100)
                _ConquestCurrencytotalWeekCap = cap * 100;
        }
...
// not used in CerberCore, but used in scripting code
...
//not used in CerberCored, function for external script library
...
void Player::SetHomebind(WorldLocation const& /*loc*/, uint32 /*area_id*/)
{
    ...
    // update sql homebind
    CharacterDatabase.PExecute("UPDATE character_homebind SET mapId = '%u', zoneId = '%u', posX = '%f', posY = '%f', posZ = '%f' WHERE guid = '%u'",
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    m_currentPetSlot = PetSlot(fields[60].GetInt32());
    m_petSlotUsed = fields[61].GetInt32();
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetCerberCoreString(LANG_NOT_EQUIPPED_ITEM);
...
void Player::_LoadVoidStorage(PreparedQueryResult result)
{
    ...
    do
    {
        ...
        if (creatorGuid > 0 && !sObjectMgr->GetPlayerByLowGUID(creatorGuid))
        {
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetCerberCoreString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement || missingLeaderAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetCerberCoreString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetCerberCoreString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
...
bool Player::_LoadHomeBind(PreparedQueryResult result)
{
    ...
    if (!ok)
    {
        ...
        CharacterDatabase.PExecute("INSERT INTO character_homebind (guid, mapId, zoneId, posX, posY, posZ) VALUES ('%u', '%u', '%u', '%f', '%f', '%f')", GetGUIDLow(), m_homebindMapId, (uint32)m_homebindAreaId, m_homebindX, m_homebindY, m_homebindZ);
    }
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
        ...
        stmt->setInt32(index++, m_currentPetSlot);
        stmt->setInt32(index++, m_petSlotUsed);
    }
    else
    {
        // Update query
...
bool Player::HandleChangeSlotModel(uint16 visSlot, uint32 newItem, uint16 pos)
{
        ...
        // впрочем, оно не будет обновл€тьс€ при смене этого самого реального итема, пока display on. Ќе критично, но потом надо переделать.
    {
        ...
        // не 0 и не 1 - следовательно, ид итема. »щем, провер€ем...
    {
        ...
        if (itemProto->Quality != ITEM_QUALITY_LEGENDARY)
            // обща€ проверка дл€ шмоток. ѕо конкретным типам - дальше.
            // спец. исключени€ типа corrupted ashbringer лучше, наверное, прописать сразу в командах  
        {
            bool condition = false;
            switch (visSlot)
            {
                case PLAYER_VISIBLE_ITEM_1_ENTRYID:
                    // head - спасибо,  эп!
...
{ 	
    ...
    CharacterDatabase.PExecute("UPDATE characters SET gender = '%u', playerBytes = '%u', playerBytes2 = '%u' WHERE guid = '%u'", gender, skin | (face << 8) | (hairStyle << 16) | (hairColor << 24), playerBytes2, GUID_LOPART(guid));
}
...
void Player::Whisper(const std::string& text, uint32 language, uint64 receiver)
{
    ...
    if (!rPlayer->isAcceptWhispers() && !isGameMaster())
    {
        ChatHandler(this).SendSysMessage(LANG_COMMAND_WHISPEROFF);
        return;
    }
...
void Player::UpdateVisibilityForPlayer()
{
    ...
    CerberCore::VisibleNotifier notifier(*this);
...
{
    uint8 v_level = victim->getLevel();
    uint8 k_grey  = CerberCore::XP::GetGrayLevel(getLevel());
...
void Player::UpdateUnderwaterState(Map* m, float x, float y, float z)
{
    ...
    if (uint32 liqEntry = liquid_status.entry)
    {
        ...
        if (liquid && liquid->SpellId)
        {
            ...
            {
                // There are only auras, so check if it's already applied
    ...
    for (EquipmentSets::iterator itr = m_EquipmentSets.begin(); itr != m_EquipmentSets.end();)
    {
        ...
        switch (eqset.state)
        {
            ...
            case EQUIPMENT_SET_CHANGED:
                ...
                trans->PAppend("UPDATE character_equipmentsets SET name='%s', iconname='%s', ignore_mask='%u', item0='%u', item1='%u', item2='%u', item3='%u', item4='%u', item5='%u', item6='%u', item7='%u', item8='%u', item9='%u', item10='%u', item11='%u', item12='%u', item13='%u', item14='%u', item15='%u', item16='%u', item17='%u', item18='%u' WHERE guid='%u' AND setguid='"UI64FMTD"' AND setindex='%u'",
                    eqset.Name.c_str(), eqset.IconName.c_str(), eqset.IgnoreMask, eqset.Items[0], eqset.Items[1], eqset.Items[2], eqset.Items[3], eqset.Items[4], eqset.Items[5], eqset.Items[6], eqset.Items[7],
            ...
            case EQUIPMENT_SET_NEW:
                CharacterDatabase.EscapeString(eqset.Name);
                CharacterDatabase.EscapeString(eqset.IconName);
                trans->PAppend("INSERT INTO character_equipmentsets VALUES ('%u', '"UI64FMTD"', '%u', '%s', '%s', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u')",
                    GetGUIDLow(), eqset.Guid, index, eqset.Name.c_str(), eqset.IconName.c_str(), eqset.IgnoreMask, eqset.Items[0], eqset.Items[1], eqset.Items[2], eqset.Items[3], eqset.Items[4], eqset.Items[5], eqset.Items[6], eqset.Items[7],
...
void Player::SendDisenchantCredit(Item* item)
{
    ...
    WorldPacket data(SMSG_DISENCHANT_CREDIT, 12 + 8);
    ...
    data << uint32(item->GetTemplate()->Class);
    data << uint32(item->GetTemplate()->Quality);
