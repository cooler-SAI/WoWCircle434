...
void Item::SaveToDB(SQLTransaction& trans)
{
    ...
    switch (uState)
    {
        ...
        {
            uint8 index = 0;
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<14>(uState == ITEM_NEW ? CHAR_REP_ITEM_INSTANCE : CHAR_UPD_ITEM_INSTANCE);
            ...
            {
                stmt = CharacterDatabase.GetPreparedStatement<2>(CHAR_UPD_GIFT_OWNER);
        ...
        case ITEM_REMOVED:
        {
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_DEL_ITEM_INSTANCE);
            stmt->setUInt32(0, guid);
            trans->Append(stmt);

            if (HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED))
            {
                stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_DEL_GIFT);
                stmt->setUInt32(0, guid);
                trans->Append(stmt);
            }
...
bool Item::LoadFromDB(uint32 guid, uint64 owner_guid, Field* fields, uint32 entry)
{
    ...
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<4>(CHAR_UPD_ITEM_INSTANCE_ON_LOAD);
...
void Item::SaveRefundDataToDB()
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement<4>(CHAR_INS_ITEM_REFUND_INSTANCE);
    stmt->setUInt32(0, GetGUIDLow());
...
void Item::ClearSoulboundTradeable(Player* currentOwner)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_DEL_ITEM_BOP_TRADE);
    stmt->setUInt32(0, GetGUIDLow());
    CharacterDatabase.Execute(stmt);
}
