...
std::string LFGMgr::GetRolesString(uint8 roles)
{
    ...
    if (roles & PLAYER_ROLE_TANK)
        rolesstr.append(sObjectMgr->GetCerberCoreStringForDBCLocale(LANG_LFG_ROLE_TANK));

    if (roles & PLAYER_ROLE_HEALER)
    {
        if (!rolesstr.empty())
            rolesstr.append(", ");
        rolesstr.append(sObjectMgr->GetCerberCoreStringForDBCLocale(LANG_LFG_ROLE_HEALER));
    }
    ...
    if (roles & PLAYER_ROLE_DAMAGE)
    {
        if (!rolesstr.empty())
            rolesstr.append(", ");
        rolesstr.append(sObjectMgr->GetCerberCoreStringForDBCLocale(LANG_LFG_ROLE_DAMAGE));
    }
    ...
    if (roles & PLAYER_ROLE_LEADER)
    {
        if (!rolesstr.empty())
            rolesstr.append(", ");
        rolesstr.append(sObjectMgr->GetCerberCoreStringForDBCLocale(LANG_LFG_ROLE_LEADER));
    }

    if (rolesstr.empty())
        rolesstr.append(sObjectMgr->GetCerberCoreStringForDBCLocale(LANG_LFG_ROLE_NONE));
...
std::string LFGMgr::GetStateString(LfgState state)
{
    ...
    return std::string(sObjectMgr->GetCerberCoreStringForDBCLocale(entry));
}
        ...
        if (hasGroup)
        {
            ...
            for (GroupReference* ref = grp->GetFirstMember(); ref != NULL; ref = ref->next())
            {
                if (Player* groupMember = ref->getSource())
                    SetQueueId<true>(groupMember->GetGUID(), queueId);
            }
        }
        else
            queueId = GetOrGenerateQueueId<false>(gguid);

        sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::Join: [" UI64FMTD "] - generate new queue Id - %u", guid, queueId);
    }
