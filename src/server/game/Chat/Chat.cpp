...
{
    const char *format = GetCerberCoreString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetCerberCoreString(int32 entry) const
{
    return m_session->GetCerberCoreString(entry);
}
...
{
    SendSysMessage(GetCerberCoreString(entry));
}
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    CerberCore::NearestGameObjectCheck check(*pl);
    CerberCore::GameObjectLastSearcher<CerberCore::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(CerberCore::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        CerberCore::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        CerberCore::GameObjectSearcher<CerberCore::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<CerberCore::GameObjectSearcher<CerberCore::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
const char *CliHandler::GetCerberCoreString(int32 entry) const
{
...
{
    return GetCerberCoreString(LANG_CONSOLE_COMMAND);
}
