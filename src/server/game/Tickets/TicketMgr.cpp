...
GmTicket::GmTicket(Player* player, WorldPacket& recvData) : _createTime(time(NULL)), _lastModifiedTime(time(NULL)), _closedBy(0), _assignedTo(0), _completed(false), _escalatedStatus(TICKET_UNASSIGNED), _haveTicket(false), _viewed(false)
{
...
void GmTicket::SaveToDB(SQLTransaction& trans) const
{
    ...
    uint8 index = 0;
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<18>(CHAR_REP_GM_TICKET);
...
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_DEL_GM_TICKET);
...
void TicketMgr::ResetTickets()
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<0>(CHAR_DEL_ALL_GM_TICKETS);
...
void TicketMgr::LoadTickets()
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<0>(CHAR_SEL_GM_TICKETS);
