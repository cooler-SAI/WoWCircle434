...
struct AchievementCriteriaEntry
{
    ...
    union
    {
        ...
        struct
        {
            uint32  groupSize;                              // 4 can be 5, 10 or 25
        } complete_raid;
        ...
        struct
        {
            uint32 type;                                    // 4, see enum EnviromentalDamage
        } death_from;
        ...
        struct
        {
            uint32 objectiveId;                             // 4
            uint32 completeCount;                           // 5
        } bg_objective;

        // ACHIEVEMENT_CRITERIA_TYPE_HONORABLE_KILL_AT_AREA = 31
        struct
        {
            uint32  areaID;                                 // 4 Reference to AreaTable.dbc
            uint32  killCount;                              // 5
        ...
        struct
        {
            uint32 mapID;                                   // 4 Reference to Map.dbc
            uint32 count;                                   // 5  Number of times that the arena must be won.
        } win_arena;
        ...
        struct
        {
            uint32  spellID;                                // 4 Reference to Map.dbc
        } learn_spell;
        ...
        {
            uint32  skillID;                                // 4
            uint32  skillLevel;                             // 5  apprentice=1, journeyman=2, expert=3, artisan=4, master=5, grand master=6
        } learn_skill_level;
        ...
        {
            uint32  factionID;                              // 4
            uint32  reputationAmount;                       // 5  Total reputation amount, so 42000 = exalted
        } gain_reputation;
        ...
        {
            uint32  unused;                                 // 4
            uint32  numberOfExaltedFactions;                // 5
       ...
       // ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED_ON_LOOT      = 51
        ...
        struct
        {
            uint32  emoteID;                                // 4 enum TextEmotes
            uint32  count;                                  // 5  count of emotes, always required special target or requirements
        } do_emote;
        ...
        struct
        {
            uint32  powerType;                              // 4 mana=0, 1=rage, 3=energy, 6=runic power
        } highest_power;

        // ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_STAT           = 97
        struct
        {
            uint32  statType;                               // 4 4=spirit, 3=int, 2=stamina, 1=agi, 0=strength
        } highest_stat;

        // ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_SPELLPOWER     = 98
        struct
        {
            uint32  spellSchool;                            // 4
        } highest_spellpower;
        ...
        struct
        {
            uint32  lootType;                               // 4 3=fishing, 2=pickpocket, 4=disentchant
            uint32  lootTypeCount;                          // 5
        } loot_type;
        ...
        struct
        {
            ...
            uint32  count;                                  // 5  main requirement count
        } raw;
    };
