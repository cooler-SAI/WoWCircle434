...
#ifndef CERBERCORE_CREATURE_TEXT_MGR_H
#define CERBERCORE_CREATURE_TEXT_MGR_H
...
void CreatureTextMgr::SendChatPacket(WorldObject* source, Builder const& builder, ChatMsg msgType, uint64 whisperGuid, TextRange range, Team team, bool gmOnly) const
{
    ...
    switch (msgType)
    {
        ...
        case CHAT_MSG_RAID_BOSS_WHISPER:
        {
            if (range == TEXT_RANGE_NORMAL || range == TEXT_RANGE_AREA) //ignores team and gmOnly
            {
    ...
    CerberCore::PlayerDistWorker<CreatureTextLocalizer<Builder> > worker(source, dist, localizer);
