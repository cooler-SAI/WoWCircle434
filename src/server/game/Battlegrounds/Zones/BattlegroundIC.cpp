...
void BattlegroundIC::DoAction(uint32 action, uint64 var)
{
    ...
    player->m_movementInfo.t_pos.m_positionX = ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionX(); 
    player->m_movementInfo.t_pos.m_positionY = ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionY(); 
    player->m_movementInfo.t_pos.m_positionZ = ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionZ(); 
    player->m_movementInfo.t_pos.m_orientation = ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetOrientation();

    if (player->TeleportTo(GetMapId(), 
        ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionX(), 
        ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionY(), 
        ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionZ(), 
        ((player->GetBGTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetOrientation(), 
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
    {
        if (nodePoint[i].gameobject_entry == target_obj->GetEntry())
        {
                ...
                nodePoint[i].banners[BANNER_H_CONTESTED] == nextBanner)
            {
                ...
                player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, 245, 1);
...
Transport* BattlegroundIC::CreateTransport(uint32 goEntry, uint32 period)
{
    ...
    if (!goinfo)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Transport ID: %u will not be loaded, gameobject_template missing", goEntry);
        delete t;
        return NULL;
    }
    ...
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Transport (path id %u) path size = 0. Transport ignored, check DBC files or transport GO data0 field.", goinfo->moTransport.taxiPathId);
        delete t;
        return NULL;
    }
