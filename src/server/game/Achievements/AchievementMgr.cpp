...
void AchievementMgr<T>::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    {
        CerberCore::AchievementChatBuilder say_builder(*GetOwner(), CHAT_MSG_GUILD_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED, achievement->ID);
        CerberCore::LocalizedPacketDo<CerberCore::AchievementChatBuilder> say_do(say_builder);
    ...
    {
        CerberCore::AchievementChatBuilder say_builder(*GetOwner(), CHAT_MSG_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED, achievement->ID);

        CellCoord p = CerberCore::ComputeCellCoord(GetOwner()->GetPositionX(), GetOwner()->GetPositionY());

        Cell cell(p);
        cell.SetNoCreate();

        CerberCore::LocalizedPacketDo<CerberCore::AchievementChatBuilder> say_do(say_builder);
        CerberCore::PlayerDistWorker<CerberCore::LocalizedPacketDo<CerberCore::AchievementChatBuilder> > say_worker(GetOwner(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), say_do);
        TypeContainerVisitor<CerberCore::PlayerDistWorker<CerberCore::LocalizedPacketDo<CerberCore::AchievementChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
void AchievementMgr<Guild>::SendCriteriaUpdate(AchievementCriteriaEntry const* entry, CriteriaProgress const* progress, uint32 /*timeElapsed*/, bool /*timedCompleted*/) const
{
    ...
    data.WriteBits(1, 21);
    data.WriteByteMask(counter[4]);
    data.WriteByteMask(counter[1]);
    data.WriteByteMask(guid[2]);
    data.WriteByteMask(counter[3]);
    data.WriteByteMask(guid[1]);
    data.WriteByteMask(counter[5]);
    data.WriteByteMask(counter[0]);
    data.WriteByteMask(guid[3]);
    data.WriteByteMask(counter[2]);
    data.WriteByteMask(guid[7]);
    data.WriteByteMask(guid[5]);
    data.WriteByteMask(guid[0]);
    data.WriteByteMask(counter[6]);
    data.WriteByteMask(guid[6]);
    data.WriteByteMask(counter[7]);
    data.WriteByteMask(guid[4]);
...

{
    ...
    for (AchievementCriteriaEntryList::const_iterator itr = criteria->begin(); itr != criteria->end(); ++itr)
    {
        ...
        criteriaBits.WriteByteMask(guid[2]);
        criteriaBits.WriteByteMask(counter[3]);
        criteriaBits.WriteByteMask(guid[1]);
        criteriaBits.WriteByteMask(counter[5]);
        criteriaBits.WriteByteMask(counter[0]);
        criteriaBits.WriteByteMask(guid[3]);
        criteriaBits.WriteByteMask(counter[2]);
        criteriaBits.WriteByteMask(guid[7]);
        criteriaBits.WriteByteMask(guid[5]);
        criteriaBits.WriteByteMask(guid[0]);
        criteriaBits.WriteByteMask(counter[6]);
        criteriaBits.WriteByteMask(guid[6]);
        criteriaBits.WriteByteMask(counter[7]);
