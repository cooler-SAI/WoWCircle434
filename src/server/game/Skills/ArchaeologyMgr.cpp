...
uint32 ArchaeologyMgr::GetSurveyBotEntry(float &orientation)
{
    ...
    for(uint8 i = 0; i < MAX_RESEARCH_SITES / 2; ++i)
    {
        ...
        uint32 site_now_1 = _player->GetUInt32Value(PLAYER_FIELD_RESERACH_SITE_1 + i) & 0xFFFF;
        uint32 site_now_2 = _player->GetUInt32Value(PLAYER_FIELD_RESERACH_SITE_1 + i) >> 16;
...
void ArchaeologyMgr::ShowResearchSites()
{
    ...
    for (ResearchSiteSet::const_iterator itr = tempSet.begin(); itr != tempSet.end(); ++itr)
    {
        ...
        {
            newvalue |= id;
            _player->SetUInt32Value(PLAYER_FIELD_RESERACH_SITE_1 + count / 2, newvalue);
        }
        else
            newvalue = id << 16;
    ...
    Log->outDebug(LOG_FILTER_SPELLS_AURAS, "rchaeologyMgr::ShowResearchSites count %u", count);
}
...
void ArchaeologyMgr::ShowResearchProjects()
{
    ...
    Log->outDebug(LOG_FILTER_SPELLS_AURAS, "rchaeologyMgr::ShowResearchProjects count %u", count);
}
...
void ArchaeologyMgr::GenerateResearchSites()
{
    ...
    Log->outDebug(LOG_FILTER_SPELLS_AURAS, "rchaeologyMgr::GenerateResearchSites");
...
void ArchaeologyMgr::GenerateResearchProjects()
{
    ...
    uint16 skill_now = _player->GetSkillValue(SKILL_ARCHAEOLOGY);
    Log->outDebug(LOG_FILTER_SPELLS_AURAS, "rchaeologyMgr::GenerateResearchSites skill_now %u", skill_now);
    if (!skill_now)
        return;
