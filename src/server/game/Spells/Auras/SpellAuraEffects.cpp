...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //281 SPELL_AURA_MOD_REPUTATION_GAIN_PCT
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //329 SPELL_AURA_MOD_RUNE_POWER_REGEN
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //335 SPELL_AURA_CAN_SEE_OR_DETECT_STEALTH
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //352 SPELL_AURA_WORGEN_CAN_TRANSFORM
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //357 SPELL_AURA_ENABLE_BOSS1_UNIT_FRAME // 97982  - client_side
    &AuraEffect::HandleNoImmediateEffect,                         //358 SPELL_AURA_WORGEN_ALTERED_FORM clientside
    &AuraEffect::HandleNoImmediateEffect,                         //359 SPELL_AURA_MOD_HEALING_DONE_ON_DEBUFF
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //363 SPELL_AURA_MOD_NEXT_SPELL
    &AuraEffect::HandleUnused,                                    //364 unused (4.3.4)
    &AuraEffect::HandleNoImmediateEffect,                         //365 SPELL_AURA_MAX_FAR_CLIP_PLANE // 106272 - client_side
    &AuraEffect::HandleAuraOverrideSpellPowerByPctAp,             //366 SPELL_AURA_OVERRIDE_SPELL_POWER_BY_AP_PCT
    &AuraEffect::HandleUnused,                                    //367 unused (4.3.4)
    &AuraEffect::HandleUnused,                                    //368 unused (4.3.4)
    &AuraEffect::HandleNULL,                                      //369 SPELL_AURA_ENABLE_POWER_BAR_TIMER
    &AuraEffect::HandleNoImmediateEffect,                         //370 SPELL_AURA_SET_FAIR_FAR_CLIP // 106786   - client_side
};
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            switch (GetMiscValue())
            {
                ...
                case FORM_BEAR:
                    ...
                    // Thick Hide
                    if (AuraEffect const* aurEff = target->GetDummyAuraEffect(SPELLFAMILY_GENERIC, 1558, 1))
                    {
                        int32 bp = aurEff->GetAmount();
...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    else
    {
        if (!target->HasAuraTypeWithMiscvalue(SPELL_AURA_MOD_INVISIBILITY, type))
        {
            ...
            // remove glow vision
            if (target->GetTypeId() == TYPEID_PLAYER && !target->HasAuraTypeMoreThanDuration(SPELL_AURA_MOD_INVISIBILITY, 0))
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->getClass() == CLASS_DRUID)
    {
        ...
        target->ToPlayer()->UpdateArmorSpecializations();
...
void AuraEffect::HandleAuraTransform(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        // polymorph case
        if ((mode & AURA_EFFECT_HANDLE_REAL) && target->GetTypeId() == TYPEID_PLAYER && target->IsPolymorphed(true))
        {
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        UnitList targets;
        CerberCore::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityRange());
        CerberCore::UnitListSearcher<CerberCore::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
        ...
        case 1733:
        {
            if (!GetAmount())
            {
                mechanic_immunity_list = (1 << MECHANIC_SNARE) | (1 << MECHANIC_ROOT)
                    ...
                    | (1 << MECHANIC_POLYMORPH) | (1 << MECHANIC_DISORIENTED)
                    | (1 << MECHANIC_FREEZE) | (1 << MECHANIC_TURN) 
                    | (1 << MECHANIC_BANISH) | (1 << MECHANIC_GRIP) ;
...
void AuraEffect::HandleModResistancePercent(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    for (int8 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; i++)
    {
        ...
        {
            float mult = target->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_RESISTANCE_PCT, 1 << i);
            target->SetModifierValue(UnitMods(UNIT_MOD_RESISTANCE_START + i), TOTAL_PCT, mult);
            if (target->GetTypeId() == TYPEID_PLAYER || target->ToCreature()->isPet())
            {
            ...
            switch (GetSpellInfo()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        CellCoord p(CerberCore::ComputeCellCoord(target->GetPositionX(), target->GetPositionY()));
                        Cell cell(p);

                        CerberCore::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck u_check(target, radius);
                        CerberCore::UnitListSearcher<CerberCore::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck> checker(target, targets, u_check);
                    ...
                    Unit* spellTarget = CerberCore::Containers::SelectRandomContainerElement(targets);
            ...
            case SPELLFAMILY_GENERIC:
            {
                switch (auraId)
                {
                    ...
                    case 83676:
                    {
                        ...
                        if (!target->HasAura(1130, caster->GetGUID()) && !target->HasAura(88691, caster->GetGUID()))
                            return;
...
void AuraEffect::HandleRaidProcFromChargeAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    if (jumps > 0)
    {
        if (Unit* caster = GetCaster())
        {
            float radius = GetSpellInfo()->Effects[GetEffIndex()].CalcRadius(caster, NULL, false);
...
void AuraEffect::HandleRaidProcFromChargeWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    if (jumps > 0)
    {
        if (Unit* caster = GetCaster())
        {
            float radius = GetSpellInfo()->Effects[GetEffIndex()].CalcRadius(caster, NULL, true);
...
void AuraEffect::HandleAuraOverrideSpellPowerByPctAp(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    if (!(mode & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraProgressBar(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        switch (GetSpellInfo()->Id)
        {
            ...
            case 106843:    // Deathwing Corrupted Blood meter
