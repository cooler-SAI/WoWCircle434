...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = CerberCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
void Map::EnsureGridCreated(const GridCoord &p)
{
    ...
    {
        CERBERCORE_GUARD(ACE_Thread_Mutex, Lock);
        if (!getNGrid(p.x_coord, p.y_coord))
        {
...
{
    CellCoord cellCoord = CerberCore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = CerberCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<CerberCore::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<CerberCore::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    CerberCore::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<CerberCore::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<CerberCore::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (std::vector<Creature*>::iterator itr = _creaturesToMove.begin(); itr != _creaturesToMove.end(); ++itr)
    {
        ...
        else
        {
            ...
            if (!CreatureRespawnRelocation(c, false))
            {
                ...
                if (c->isPet())
                    ((Pet*)c)->Remove(PET_SLOT_OTHER_PET, true);
                else
...
{
    cell.SetNoCreate();
    CerberCore::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<CerberCore::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    CerberCore::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<CerberCore::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<CerberCore::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridCoord p = CerberCore::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridCoord p2 = CerberCore::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
