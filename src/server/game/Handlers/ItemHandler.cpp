    ...
    for (uint32 slot = 0; slot < tempCount; ++slot)
    {
        ...
        if (vendorItem->Type == ITEM_VENDOR_TYPE_ITEM)
        {
            ...
            if (!_player->isGameMaster()) // ignore conditions if GM on
            {
                ...
                {
                    sLog->outError(LOG_FILTER_CONDITIONSYS, "SendListInventory: conditions not met for creature entry %u item %u", vendor->GetEntry(), vendorItem->item);
                    continue;
                }
        ...
        else if (vendorItem->Type == ITEM_VENDOR_TYPE_CURRENCY)
        {
            ...
            // if (!unk "enabler") data << uint32(something);
            itemsData << int32(-1);
            itemsData << uint32(vendorItem->maxcount * currencyTemplate->GetPrecision());
        }
        // else error
    }
...
void WorldSession::HandleTransmogrifyItems(WorldPacket& recvData)
{
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        if (newEntries[i])
        {
            ...
            if (!player->HasItemCount(newEntries[i]))
            {
                sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify to an item he has not.", player->GetGUIDLow(), player->GetName(), newEntries[i]);
                return;
            }
        }
