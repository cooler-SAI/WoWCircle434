...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(CHAR_SEL_PET_SLOTS_CHANGE, "SELECT slot, entry, id FROM character_pet WHERE owner = ? AND id = ?", CONNECTION_ASYNC);
    ...
    "health, power1, power2, power3, power4, power5, instance_id, speccount, activespec, exploredZones, equipmentCache, knownTitles, actionBars, grantableLevels, currentPetSlot, petSlotUsed FROM characters WHERE guid = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHARACTER_HOMEBIND, "SELECT map, zone, position_x, position_y, position_z FROM character_homebind WHERE guid = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_ADD_ITEM_DONATE, "REPLACE INTO character_donate (`owner_guid`, `itemguid`, `itemEntry`, `efircount`, `count`) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_UPD_GUILD_MEMBER_XP, "UPDATE guild_member SET total_activity = ?, week_activity = ? WHERE guid = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RESET_WEEKLY_REPUTATION, "UPDATE guild_member SET week_rep = 0", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_WEEKLY_REPUTATION, "UPDATE guild_member SET week_rep = ? WHERE guildid = ? AND guid = ?", CONNECTION_ASYNC);
    ...
    // Currency
    PREPARE_STATEMENT(CHAR_SEL_PLAYER_CURRENCY, "SELECT currency, week_count, total_count, season_count, flags FROM character_currency WHERE guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_PLAYER_CURRENCY, "UPDATE character_currency SET week_count = ?, total_count = ?, season_count = ?, flags = ? WHERE guid = ? AND currency = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_REP_PLAYER_CURRENCY, "REPLACE INTO character_currency (guid, currency, week_count, total_count, season_count, flags) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    // Petitions
    PREPARE_STATEMENT(CHAR_SEL_PETITION, "SELECT ownerguid, name FROM petition WHERE petitionguid = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(CHAR_DEL_PETITION_SIGNATURE, "DELETE FROM petition_sign WHERE playerguid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_SEL_PETITION_BY_OWNER, "SELECT petitionguid FROM petition WHERE ownerguid = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_SEL_PETITION_SIGNATURES, "SELECT ownerguid, (SELECT COUNT(playerguid) FROM petition_sign WHERE petition_sign.petitionguid = ?) AS signs FROM petition WHERE petitionguid = ?", CONNECTION_SYNCH);
    ...
    // Character homebind
    PREPARE_STATEMENT(CHAR_INS_PLAYER_HOMEBIND, "INSERT INTO character_homebind (guid, map, zone, position_x, position_y, position_z) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_UPD_PLAYER_HOMEBIND, "UPDATE character_homebind SET map = ?, zone = ?, position_x = ?, position_y = ?, position_z = ? WHERE guid = ?", CONNECTION_ASYNC)
    ...
    "power4, power5, latency, speccount, activespec, exploredZones, equipmentCache, knownTitles, actionBars, grantableLevels, currentPetSlot, petSlotUsed) VALUES "
    "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    ...
    "equipmentCache=?,knownTitles=?,actionBars=?,grantableLevels=?,online=?,currentPetSlot=?,petSlotUsed=? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHAR_HOMEBIND, "SELECT map, zone, position_x, position_y, position_z FROM character_homebind WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(CHAR_INS_PETITION, "INSERT INTO petition (ownerguid, petitionguid, name) VALUES (?, ?, ?)", CONNECTION_ASYNC);
