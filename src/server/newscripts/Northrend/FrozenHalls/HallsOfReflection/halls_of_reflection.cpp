            ...
            while (uint32 event = _events.ExecuteEvent())
            {
                switch (event)
                {
                    ...
                    case EVENT_ESCAPE_3:
                        ...
                        {
                            if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                                me->AI()->DoCast(lichking, SPELL_JAINA_ICEPRISON, true);
                            else
                                me->AI()->DoCast(lichking, SPELL_SYLVANAS_DARKBINDING, true);
                        }
                    ...
                    case EVENT_ESCAPE_6:
                        ...
                        if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                            ...
                            DoCastAOE(SPELL_FINDING_SYLVANAS_CREDIT);
                        break;
                    case EVENT_ESCAPE_7:
                        if (Creature* lichking = me->GetCreature(*me, _lichkingGUID))
                        {
                    ...
                    case EVENT_ESCAPE_27:
                        if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                            me->SummonGameObject(IsHeroic() ? GO_CAPTAIN_CHEST_4 : GO_CAPTAIN_CHEST_2, ChestPos.GetPositionX(), ChestPos.GetPositionY(), ChestPos.GetPositionZ(), ChestPos.GetOrientation(), 0, 0, 0, 0, 720000);
                        else
