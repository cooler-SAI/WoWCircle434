        ...
        class spell_malygos_arcane_storm_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (GetSpellInfo()->Id == SPELL_ARCANE_STORM_P_III)
                {
                    ...
                    CerberCore::Containers::RandomResizeList(targets, check, (malygos->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL ? 4 : 10));
                }
                else
                    CerberCore::Containers::RandomResizeList(targets, (malygos->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL ? 4 : 10));
            }
        ...
        class spell_arcane_overload_SpellScript : public SpellScript
        {
            ...
            void ResizeEffectRadiusTargetChecker(std::list<WorldObject*>& targets)
            {
                ...
                targets.remove_if(ExactDistanceCheck(arcaneOverload,
                    GetSpellInfo()->Effects[EFFECT_0].CalcRadius(arcaneOverload, NULL, false) * arcaneOverload->GetFloatValue(OBJECT_FIELD_SCALE_X)));
            }
            ...
            void FilterMeleeHoverDiskPassangers(std::list<WorldObject*>& targets)
            {
                ...
                if (targets.size() > 1)
                    if (malygos && malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID))
                        targets.remove_if(CerberCore::ObjectGUIDCheck(malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID)));
                ...
                if (!targets.empty())
                    targets.remove_if(CerberCore::UnitAuraCheck(true, SPELL_ARCANE_BARRAGE_DAMAGE));
    ...
    public:
        ...
        class spell_malygos_surge_of_power_warning_selector_25_SpellScript : public SpellScript
        {
            ...
            void SendThreeTargets(std::list<WorldObject*>& targets)
            {
                ...
                CerberCore::Containers::RandomResizeList(selectedTargets, 3);
