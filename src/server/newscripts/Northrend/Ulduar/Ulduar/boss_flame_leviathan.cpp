                ...
                if (targets.HasTraj())
                    if (Vehicle* vehicle = GetCaster()->GetVehicleKit())
                        if (Unit* passenger = vehicle->GetPassenger(damage - 1))
                        {
                            ...
                            CerberCore::WorldObjectSpellAreaTargetCheck check(99, GetExplTargetDest(), GetCaster(), GetCaster(), GetSpellInfo(), TARGET_CHECK_DEFAULT, NULL);
                            CerberCore::WorldObjectListSearcher<CerberCore::WorldObjectSpellAreaTargetCheck> searcher(GetCaster(), targetList, check);
                            ...
                            if (target && target->IsWithinDist2d(targets.GetDstPos(), GetSpellInfo()->Effects[effIndex].CalcRadius(NULL, NULL, true) * 2)) // now we use *2 because the location of the seat is not correct
