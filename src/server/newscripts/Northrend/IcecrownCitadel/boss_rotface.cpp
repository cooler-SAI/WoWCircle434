                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_MUTATED_INFECTION:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, NonTankTargetSelector(me)))
    ...
    public:
        ...
        struct npc_big_oozeAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* /*summoner*/)
            {
                ...
                events.ScheduleEvent(EVENT_STICKY_OOZE, 5000);
                events.ScheduleEvent(EVENT_ENABLE_ATTACK, 3000);
                // register in Rotface's summons - not summoned with Rotface as owner
                if (Creature* rotface = Unit::GetCreature(*me, instance->GetData64(DATA_ROTFACE)))
                    rotface->AI()->JustSummoned(me);
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (me->IsVisible() && _canAttackMelee)
                    DoMeleeAttackIfReady();
            }
    ...
    public:
        ...
        class spell_rotface_ooze_flood_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                triggers.sort(CerberCore::ObjectDistanceOrderPred(GetHitUnit()));
    ...
    public:
        ...
        class spell_rotface_mutated_infection_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                targets.remove_if (CerberCore::UnitAuraCheck(true, GetSpellInfo()->Id));
                if (targets.empty())
                    return;
