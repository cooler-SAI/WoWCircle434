...
void StartFlyShip(Transport* ship)
{
    ...
    ship->SetUInt32Value(GAMEOBJECT_DYNAMIC, 0x10830010); // Seen in sniffs
    ...
    ship->GenerateWaypoints(goinfo->moTransport.taxiPathId, mapsUsed);
...
{
    if (Creature* capitan = ObjectAccessor::GetCreature(*me, instance->GetData64(data)))
...
void RestartEvent(Transport* firstShip, Transport* secondShip, Map* instance, uint64 teamInInstance)
{
    ...
    {
        if (Transport* allianceHammerShip = sMapMgr->LoadTransportInMap(instance, GO_ORGRIM_S_HAMMER_ALLIANCE_ICC, 108000))
        {
            ...
            allianceHammerShip->SetStopped(true);
            allianceHammerShip->BuildStopMovePacket(allianceHammerShip->GetMap());
        }

        if (Transport* allianceSkyBreakerShip = sMapMgr->LoadTransportInMap(instance, GO_THE_SKYBREAKER_ALLIANCE_ICC, 108000))
        {
            ...
            allianceSkyBreakerShip->SetStopped(true);
            allianceSkyBreakerShip->BuildStopMovePacket(allianceSkyBreakerShip->GetMap());
        }
    }
    else if (teamInInstance == HORDE)
    {
        if (Transport* hordeSkyBreakerShip = sMapMgr->LoadTransportInMap(instance, GO_THE_SKYBREAKER_HORDE_ICC, 77800))
        {
            ...
            hordeSkyBreakerShip->SetStopped(true);
            hordeSkyBreakerShip->BuildStopMovePacket(hordeSkyBreakerShip->GetMap());
        }

        if (Transport* hordeHammerShip = sMapMgr->LoadTransportInMap(instance,GO_ORGRIM_S_HAMMER_HORDE_ICC, 77800))
        {
        ...
        struct npc_muradin_gunshipAI : public ScriptedAI
        {
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    ...
                    case ACTION_DONE:
                        if (Creature* allianceBoss = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_SKYBREAKER_BOSS)))
                        {
                            _instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, allianceBoss);
                            allianceBoss->DespawnOrUnsummon();
                        }

                        if (Creature* hordeBoss = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_ORGRIMMAR_HAMMER_BOSS)))
                        {
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    ...
                    case ACTION_BATTLE_EVENT:
                    {
                        ...
                        Creature* hordeBoss = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_ORGRIMMAR_HAMMER_BOSS));
                        Creature* allianceBoss = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_SKYBREAKER_BOSS));
