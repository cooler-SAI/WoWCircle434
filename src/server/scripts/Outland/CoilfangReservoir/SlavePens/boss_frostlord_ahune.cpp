...
enum Events
{
    ...
    EVENT_SUBMERGE              = 3,
    EVENT_SUBMERGED             = 4,
    EVENT_EARTHEN_RING_ATTACK   = 5,
    EVENT_SUMMON_HAILSTONE      = 6,
    EVENT_SUMMON_COLDWAVE       = 7,
    EVENT_ICE_SPEAR             = 8,
};
...
class npc_frostlord_ahune : public CreatureScript
{
    public:
        ...
        {
            return new npc_frostlord_ahuneAI(pCreature);
        }

        struct npc_frostlord_ahuneAI : public Scripted_NoMovementAI
        {
            npc_frostlord_ahuneAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature), summons(me) 
            {
            ...
            {
                if (Player* pPlayer = me->SelectNearestPlayer(100.0f))
                    pPlayer->SummonGameObject(GO_ICE_CHEST, -97.095f, -203.866f, -1.191f, 1.5f, 0, 0, 0, 0, 0);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_EMERGE:
                            ...
                            DoCast(SPELL_EMERGE);
                            events.ScheduleEvent(EVENT_SUBMERGE, 95000);
                        ...
                        case EVENT_SUBMERGE:
                            ...
                            events.CancelEvent(EVENT_SUMMON_HAILSTONE);
                            events.CancelEvent(EVENT_SUMMON_COLDWAVE);
                        ...
                        case EVENT_SUMMON_COLDWAVE:
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_COLDWAVE, 10000);
                            break;
...
class npc_ahune_frozen_core : public CreatureScript
{
    public:
        ...
        struct npc_ahune_frozen_coreAI : public Scripted_NoMovementAI
        {
            npc_ahune_frozen_coreAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
            {
                me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
                me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_GRIP, true);
                me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_STUN, true);
            ...
            {
                if (Creature* pAhune = me->FindNearestCreature(NPC_FROSTLORD_AHUNE, 100.0f))
                    if (pAhune->isAlive())
                        pAhune->DealDamage(pAhune, damage);
            }
    ...
    public:
        ...
        bool OnGossipSelect(Player* player, GameObject* go, uint32 /*sender*/, uint32 action)
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF)
            {
                ...
                {
                    if (Creature* pLuma = go->FindNearestCreature(NPC_LUMA_SKYMOTHER, 100.0f))
                        pLuma->AI()->Talk(0);
