...
public:
    ...
    static bool HandleBanInfoCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_SEL_BANINFO);
    ...
    {
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement<0>(LOGIN_DEL_EXPIRED_IP_BANS);
        ...
        if (filter.empty())
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement<0>(LOGIN_SEL_ACCOUNT_BANNED_ALL);
            result = LoginDatabase.Query(stmt);
        }
        else
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement<1>(LOGIN_SEL_ACCOUNT_BANNED_BY_USERNAME);
    ...
    static bool HandleBanListCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement<1>(CHAR_SEL_GUID_BY_NAME_FILTER);
        stmt->setString(0, filter);
        PreparedQueryResult result = CharacterDatabase.Query(stmt);
        if (!result)
        {
        ...
        if (handler->GetSession())
        {
            ...
            {
                Field* fields = result->Fetch();
                PreparedStatement* stmt2 = CharacterDatabase.GetPreparedStatement<1>(CHAR_SEL_BANNED_NAME);
        ...
        else
        {
            ...
            do
            {
                ...
                PreparedStatement* stmt2 = CharacterDatabase.GetPreparedStatement<1>(CHAR_SEL_BANINFO_LIST);
                stmt2->setUInt32(0, fields[0].GetUInt32());
    ...
    static bool HandleBanListIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (filter.empty())
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement<0>(LOGIN_SEL_IP_BANNED_ALL);
            result = LoginDatabase.Query(stmt);
        }
        else
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement<1>(LOGIN_SEL_IP_BANNED_BY_IP);
            stmt->setString(0, filter);
            result = LoginDatabase.Query(stmt);
        }
