...
public:
    ...
    static bool HandleDebugSendBuyErrorCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->GetSession()->GetPlayer()->SendBuyError(msg, 0, 0);
        return true;
    }
    ...
    static bool HandleDebugEnterVehicleCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            CerberCore::AllCreaturesOfEntryInRange check(handler->GetSession()->GetPlayer(), entry, 20.0f);
            CerberCore::CreatureSearcher<CerberCore::AllCreaturesOfEntryInRange> searcher(handler->GetSession()->GetPlayer(), passenger, check);
    ...
    static bool HandleDebugHasAuraCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (Aura * aura = unit->GetAura(auraid))
        {
            ...
            char const* talentStr = handler->GetCerberCoreString(LANG_TALENT);
            char const* passiveStr = handler->GetCerberCoreString(LANG_PASSIVE);
