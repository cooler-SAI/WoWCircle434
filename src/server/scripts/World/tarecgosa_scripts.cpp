...
class npc_hallegosa : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return new npc_hallegosaAI(pCreature);
        }
        ...
        bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*sender*/, uint32 /*action*/)
        {
            ...
            pCreature->GetMotionMaster()->MovePoint(POINT_HALLEGOSA_1, ((pCreature->GetMapId() == 1) ? finalPos2[0] : finalPos1[0]));
            return true;
        }

        struct npc_hallegosaAI : ScriptedAI
        {
            npc_hallegosaAI(Creature* pCreature) : ScriptedAI(pCreature) 
            {
                curPoint = 0;
                me->RemoveAllAuras();
                me->SetCanFly(false);
            ...
            void sQuestAccept(Player* pPlayer, const Quest quest)
            {
...
class npc_kalecgos_53392 : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return new npc_kalecgos_53392AI(pCreature);
        }

        struct npc_kalecgos_53392AI : ScriptedAI
        {
            npc_kalecgos_53392AI(Creature* pCreature) : ScriptedAI(pCreature) 
            {
                me->setActive(true);
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    {
                        case EVENT_TALK_1:
                            Talk(SAY_53392_1, 0, 2);
                            events.ScheduleEvent(EVENT_TALK_2, 6000);
                            break;
                        case EVENT_TALK_2:
                            Talk(SAY_53392_2, 0, 2);
                            if (me->GetMapId() == 1)
                            {
                            ...
                            break;
                        case EVENT_TALK_3:
                            Talk(SAY_53392_3, 0, 2);
                            events.ScheduleEvent(EVENT_TALK_4, 6000);
                            break;
                        case EVENT_TALK_4:
                            Talk(SAY_53392_4, 0, 2);
                        ...
                        case EVENT_TALK_5:
                            ...
                            {
                                pStalker->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP | UNIT_NPC_FLAG_QUESTGIVER);
                                pStalker->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 0, 69859);
                        ...
                        case EVENT_ESSENCE_2:
                            ...
                            break;
                        case EVENT_TALK_6:
                            Talk(SAY_53392_6, 0, 2);
                            events.ScheduleEvent(EVENT_TALK_7, 13000);
                            break;
                        case EVENT_TALK_7:
                            Talk(SAY_53392_7, 0, 2);
                            if (Creature* pStalker = me->FindNearestCreature(NPC_STALKER_53422, 100.0f))
                                pStalker->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER);
                            events.ScheduleEvent(EVENT_TALK_8, 7000);
                            break;
                        case EVENT_TALK_8:
                            Talk(SAY_53392_8, 0, 2);
...
class npc_tarecgosa_53396 : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return new npc_tarecgosa_53396AI(pCreature);
        }

        struct npc_tarecgosa_53396AI : ScriptedAI
        {
            npc_tarecgosa_53396AI(Creature* pCreature) : ScriptedAI(pCreature) 
            {
                me->setActive(true);
            }
        };
...
class npc_blue_dragon_53394 : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return new npc_blue_dragon_53394AI(pCreature);
        }

        struct npc_blue_dragon_53394AI : ScriptedAI
        {
            npc_blue_dragon_53394AI(Creature* pCreature) : ScriptedAI(pCreature) 
            {
                me->setActive(true);
            }
        };
...
class npc_stalker_53422 : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return new npc_stalker_53422AI(pCreature);
        }

        struct npc_stalker_53422AI : ScriptedAI
        {
            npc_stalker_53422AI(Creature* pCreature) : ScriptedAI(pCreature) 
            {
                me->setActive(true);
            }

            void sQuestReward(Player* pPlayer, const Quest* quest, uint32 data)
            {
