        ...
        void MovementInform(uint32 type, uint32 id)
        {
            if (id == EVENT_JUMP)
            {
                if (Unit* pTarget = me->getVictim())
                    DoCast(pTarget, SPELL_FACE_RAGE_2, true);
            }
        }
    ...
    struct npc_shannox_crystal_prison_trapAI : public Scripted_NoMovementAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            else if (bReady && !bExplode)
            {
                ...
                {
                    if (!pRiplimb->HasAura(SPELL_WARRY) && !pRiplimb->HasAura(SPELL_WARRY_25) && 
                        !pRiplimb->HasAura(SPELL_WARRY_10H) && !pRiplimb->HasAura(SPELL_WARRY_25H))
                    {
                        bExplode = true;
                ...
                {
                    if (!pRageface->HasAura(SPELL_WARRY) && !pRageface->HasAura(SPELL_WARRY_25) &&
                        !pRageface->HasAura(SPELL_WARRY_10H) && !pRageface->HasAura(SPELL_WARRY_25H))
                    {
                        bExplode = true;
