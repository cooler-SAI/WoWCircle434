                    ...
                    case DATA_INFINITE_EVENT:
                        ...
                        switch (data)
                        {
                            case DONE:
                            {
                                ...
                                if (!players.isEmpty())
                                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                        itr->getSource()->CompletedAchievement(sAchievementStore.LookupEntry(ACHIEVEMENT_CULLING_TIME));
                                break;
                            }
                            case FAIL:
                                DoUpdateWorldState(WORLDSTATE_TIME_GUARDIAN_SHOW, 0);
                    ...
                    case DATA_ZOMBIEFEST:
                        ...
                        switch(data)
                        {
                            case DONE:
                            {
                                ...
                                if (!players.isEmpty())
                                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                        itr->getSource()->CompletedAchievement(sAchievementStore.LookupEntry(ACHIEVEMENT_ZOMBIEFEST));
                                _zombieFest = data;
                                break;
                            }
                            case IN_PROGRESS:
