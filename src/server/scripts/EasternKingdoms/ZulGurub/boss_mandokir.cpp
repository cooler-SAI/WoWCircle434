            ...
            void DoAction(const int32 action)
            {
                switch (action)
                {
                    ...
                    case ACTION_START_REVIVE:
                    {
                        ...
                        creatures.remove_if(CerberCore::AnyDeadUnitCheck());
                        creatures.remove_if(CerberCore::UnitAuraCheck(true, SPELL_OHGAN_ORDERS_TRIGGER));
                        CerberCore::Containers::RandomResizeList(creatures, 1);
                        if (creatures.empty())
                            return;
