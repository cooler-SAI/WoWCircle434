...
enum Events
{
    ...
    //electron
    EVENT_ELECTRICAL_DISCHARGE            = 3,
    EVENT_LIGHTNING_CONDUCTOR            = 4,

    //magmatron
    EVENT_ACQUIRING_TARGET                = 5,
    EVENT_INSENERATION_SECURITY_MISSURE    = 6,

    //toxitron
    EVENT_CHEMICAL_BOMB                    = 7,
    EVENT_POISON_PROTOCOL                = 8,
};
...
public:
    ...
    struct boss_omnotronAI : public ScriptedAI
    {
        ...
        Creature* GetOmnotronNext()
        {
            if (!pInstance)
                return 0;

            uint64 m_uiGuids[4];
            m_uiGuids[0] = pInstance->GetData64(DATA_ARCANOTRON);
            m_uiGuids[1] = pInstance->GetData64(DATA_ELECTRON);
            m_uiGuids[2] = pInstance->GetData64(DATA_MAGMATRON);
            m_uiGuids[3] = pInstance->GetData64(DATA_TOXITRON);

            std::vector<Creature*> omnotrons;
            for (uint8 i = 0; i < 4; ++i)
            {
                if (Creature* omnotron = Unit::GetCreature(*me, m_uiGuids[i]))
                    if (omnotron->HasAura(SPELL_INACTIVE_AURA))
                        omnotrons.push_back(omnotron);
            }
            return omnotrons.size() ? omnotrons[urand(0, omnotrons.size() - 1)] : 0;
        }
        ...
        void DoAction(const int32 action)
        {
            ...
            {
            case ACTION_NEXT:
                if (Creature* omnotronnext = GetOmnotronNext())
                {
                    omnotronnext->GetAI()->DoAction(ACTION_ACTIVE);
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
                        omnotronnext->AI()->AttackStart(target);
...
public:
    ...
    {
        boss_arcanotronAI(Creature* pCreature) : BossAI(pCreature, DATA_OMNOTRON), summons(me)
        {
        ...
        void DoAction(const int32 action)
        {
            switch (action)
            {
                case ACTION_ACTIVE:
                    stage = 1;
                    me->RemoveAurasDueToSpell(SPELL_INACTIVE_AURA);
                    me->SetPower(POWER_ENERGY, 100);
                    DoCast(me,SPELL_ACTIVE, false);
                    break;
            }
        }
...
public:
    ...
    {
        boss_electronAI(Creature* pCreature) : BossAI(pCreature, DATA_OMNOTRON), summons(me)
        {
            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
            me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_GRIP, true);
            me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_STUN, true);
...
public:
    ...
    {
        boss_magmatronAI(Creature* pCreature) : BossAI(pCreature, DATA_OMNOTRON), summons(me)
        {
            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
            me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_GRIP, true);
            me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_STUN, true);
...
public:
    ...
    {
        boss_toxitronAI(Creature* pCreature) : BossAI(pCreature, DATA_OMNOTRON), summons(me)
        {
            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
            me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_GRIP, true);
            me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_STUN, true);
