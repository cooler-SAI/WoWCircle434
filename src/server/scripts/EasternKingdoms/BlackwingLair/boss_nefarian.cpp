...
public:
    ...
    struct boss_victor_nefariusAI : public BossAI
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (UpdateVictim() && SpawnedAdds <= ADDS_COUNT)
            {
                events.Update(diff);
                            ...
                            for (uint8 i=0; i<2; ++i)
                            {
                                ...
                                if (++SpawnedAdds >= ADDS_COUNT)
                                {
                                    if (Creature* nefarian = me->SummonCreature(NPC_NEFARIAN, NefarianLoc[0]))
                                    {
                                        ...
                                        if (nefarian->SelectVictim())
                                            nefarian->Attack(nefarian->getVictim(), true);
                                        nefarian->AI()->DoCastAOE(SPELL_SHADOWFLAME_INITIAL);
                                        //nefarian->GetMotionMaster()->MovePoint(1001, NefarianLoc[1]);
                                    }
