...
Position const RagnarosSummonPosition = {1075.2f,   -57.849f,   55.4243f,   3.15905f    };
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/)
        {
            if (InstanceScript* instance = player->GetInstanceScript())
                if (!ObjectAccessor::GetCreature(*player, instance->GetData64(DATA_RAGNAROS)))
                    player->SummonCreature(NPC_RAGNAROS_CATA, RagnarosSummonPosition, TEMPSUMMON_MANUAL_DESPAWN, 0);
            return true;
        }
};
...
class npc_ragnaros_lava : public CreatureScript
{
public:
    npc_ragnaros_lava() : CreatureScript("npc_ragnaros_lava") { }

    struct npc_ragnaros_lavaAI : public ScriptedAI
    {
        npc_ragnaros_lavaAI(Creature* creature) : ScriptedAI(creature) {}
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_ragnaros_lavaAI (creature);
    }
};
...
class spell_boss_raise_lava : public SpellScriptLoader
{
    public:
        ...
        class spell_boss_raise_lava_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_boss_raise_lava_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
            }
        };
